...
class PickabooSpider(scrapy.Spider):
	...
	#add to items[key] in python dict structure
        ...
        item['img_url'] = response.xpath('//*[@id="owl-demo"]//img/@src').extract_first()
        item['image_urls'] = [item['img_url']]
        #item['image_urls'] = response.xpath('//*[@id="owl-demo"]//img/@src').extract_first() #[0].extract()
        ...
        item['rating'] = response.xpath('//div[@class="mobile-pdt-rating"]//p[@class="amount-mobile"]/text()').extract()
